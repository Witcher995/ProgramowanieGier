...
public class FountainGLRenderer extends GLSurfaceView implements Renderer
{
    ...
    public FountainGLRenderer(Activity pActivity)
    {
        super(pActivity);
    ...
    public void onDrawFrame(GL10 gl1)
    {
		...
		/*
		//=== Multipass Render ===
		//remove other calls to gl.glMatrixMode and DrawSceneObjects
		// --- Draw far objects ---
		gl.glPushMatrix();
		...
		gl.glFrustumf(-mScrRatio, mScrRatio, -1f, 1f, 1f, 98);
		gl.glMatrixMode(GL10.GL_MODELVIEW);
		gl.glLoadIdentity();   // reset the matrix to its default state
		GLU.gluLookAt(gl, mCamXpos, mCamYpos, mCamZpos, mTargetX, mTargetY, mTargetZ, 0f, 100.0f, 0.0f);
    ...
    void DrawQuad(GL11 gl, float[] pX, float[] pY, float[] pZ) //clockwise starting top left
    {
        ...
        FloatBuffer buffer;
        ByteBuffer vbb = ByteBuffer.allocateDirect(vtx.length * 4); //float is 4 bytes
        vbb.order(ByteOrder.nativeOrder()); //use the device hardware's native byte order
        buffer = vbb.asFloatBuffer(); //create a floating point buffer from the ByteBuffer
        buffer.put(vtx); //add the coordinates to the FloatBuffer
        buffer.position(0); //set the buffer to read the first coordinate
        gl.glVertexPointer(3, GL11.GL_FLOAT, 0, buffer); //3 values per vertex [x/y/z]
